...
export class ChromeMultiPortMessageBusDriver implements MessageBusDriver{
    ...
    addPort(port: chrome.runtime.Port){
        ...
        port.onDisconnect.addListener(() => this.ports = this.ports.filter(p => p !== port));
    ...
    setMessageHandler(handler: (message: ProtocolMessage) => void): void {
        this.ports.forEach(port => port.onMessage.removeListener(this.handler));
    ...
    connect({descriptor}){
        try{
            if(!chrome.runtime?.connect){
                throw new Error('Failed to connect to chrome message passing capabilities')
            }
