...
export type MessageListener<T extends ProtocolMessage> = (res: T) => MessageListener<T> | null;
...
export const kTimes = <T extends ProtocolMessage>(times: number, handler: MessageListener<T>) => {
    return (message: T) => {
...
export const oneOf = (handlers: Partial<Record<ProtocolMessage['type'], MessageListener<ProtocolMessage>>>) => {
