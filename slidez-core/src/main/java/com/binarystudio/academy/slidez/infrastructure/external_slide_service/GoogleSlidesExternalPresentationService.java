...
public class GoogleSlidesExternalPresentationService {
	...
	public String createSlide(UUID actorId, String presentationId, String slideId, String text) throws IOException {
		...
		request.setRequests(List.of(new Request().setCreateSlide(new CreateSlideRequest().setObjectId(slideId)),
				new Request().setCreateShape(new CreateShapeRequest().setObjectId(textBoxId).setShapeType("TEXT_BOX")
						.setElementProperties(new PageElementProperties().setPageObjectId(slideId)
								.setSize(new Size().setHeight(pt350).setWidth(pt350))
								.setTransform(new AffineTransform().setScaleX(1.0).setScaleY(1.0).setTranslateX(10.0)
										.setTranslateY(30.0).setUnit("PT")))),
				new Request().setInsertText(new InsertTextRequest().setText(text).setObjectId(textBoxId))));
